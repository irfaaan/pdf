...
**With AI vetting enabled, ~10–20s later the same message is edited to add the EXACT AI verdict line** (reasoning/thinking is stripped server-side — never posted):

```
🤖 AI vetting: CONFIRM | downtrend continuation at EMA21 with order block confluence
```

The model's chain-of-thought (e.g. `<thinking>…</thinking>` blocks or the API's `reasoning` field) is logged at DEBUG level in `data/logs/gold_bot.log` and **never** appears in Telegram. Only the exact one-line verdict — `CONFIRM | …`, `REJECT | …` or `NEUTRAL | …` — is appended.

**On-demand exact response** — send `/aicheck` to the bot at any time to get the model's exact verdict on the current market:

```
🤖 Asking nvidia/nemotron-3-ultra-550b-a55b:free for its verdict…
🤖 AI verdict (nvidia/nemotron-3-ultra-550b-a55b:free):
CONFIRM | momentum aligned with VWAP, clean long continuation
```
